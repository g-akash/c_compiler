...
parse.cc Parser.h Parserbase.h Parser.ih: parse.y
	...
	sed -i '/#include <iostream>/a #include "ast.h"' Parserbase.h;
	# sed -i '/#include <iostream>/a #include "symtab.h"' Parserbase.h;
	sed -i '\|STYPE__     d_val__;| d' Parserbase.h;
	sed -i '/STYPE__     d_nextVal__;/a \\tpublic: static STYPE__  d_val__; ' Parserbase.h;
	sed -i '/include "Parser.ih"/a #include "ast.cpp"' parse.cc;
	# sed -i '/include "Parser.ih"/a #include "symtab.cpp"' parse.cc;
	sed -i '\|void Parser::print__()| i Meta__::SType ParserBase::d_val__; ' parse.cc
	sed -i '\|typedef Meta__::SType STYPE__;| a \\tpublic: static int linenum;' Parserbase.h 
	sed -i '\|void Parser::print__()| i int ParserBase::linenum = 1;' parse.cc
